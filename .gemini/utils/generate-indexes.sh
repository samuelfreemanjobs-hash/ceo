...
extract_agent_metadata() {
    ...
    local name="" agent_id="" title="" when_to_use=""

    # Frontmatter style (--- delimited)
    if head -1 "$file" | grep -q '^---'; then
        local fm
        fm=$(awk 'BEGIN{p=0} /^---$/{p++; next} p==1{print}' "$file")
        name=$(echo "$fm" | grep -E '^name:' | head -1 | sed 's/^name:\s*//' | tr -d '"' | xargs)
        title=$(echo "$fm" | grep -E '^title:' | head -1 | sed 's/^title:\s*//' | tr -d '"' | xargs)
        when_to_use=$(echo "$fm" | grep -E '^description:' | head -1 | sed 's/^description:\s*//' | tr -d '"' | xargs)
        [[ -n "$name" ]] && id="$name"
    else
        # Codex-style YAML block
        local yaml_block
        yaml_block=$(sed -n '/```yaml/,/```/p' "$file" | grep -v '```')
        name=$(echo "$yaml_block" | grep -E '^\s*name:' | head -1 | sed 's/.*name:\s*//' | tr -d '"' | xargs)
        agent_id=$(echo "$yaml_block" | grep -E '^\s*id:' | head -1 | sed 's/.*id:\s*//' | tr -d '"' | xargs)
        title=$(echo "$yaml_block" | grep -E '^\s*title:' | head -1 | sed 's/.*title:\s*//' | tr -d '"' | xargs)
        when_to_use=$(echo "$yaml_block" | grep -E '^\s*whenToUse:' | head -1 | sed 's/.*whenToUse:\s*//' | tr -d '"')
