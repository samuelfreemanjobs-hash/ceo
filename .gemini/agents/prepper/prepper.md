...
**Identity layer:** Load `.gemini/agents/prepper/SOUL.md` for personality, tone, and identity constraints. Pairs with this operational spec.

**Skills layer:** Load `.gemini/agents/prepper/SKILLS.md` for capability triggers, task workflows, and checklists.

**Duties layer:** Load `.gemini/agents/prepper/DUTIES.md` for deliverables, SLAs, and definition of done.

**Rules layer:** Load all files in `.gemini/agents/prepper/rules/` — hard stops; violations require halt and report.

**Memory layer:** Read/write `.gemini/agents/prepper/memory/` per README; shared context in `.ai/data/kb.yaml`.

**Subagents layer:** Load `.gemini/agents/prepper/SUBAGENTS.md` for delegation map and Task tool templates.
...
1. Follow Analysis Task: Use `.gemini/tasks/analyze-project-context.yaml`

2. Read Project Configuration:
   - Core config: `.gemini/core-config.xml`
...
4. Create Analysis Report:
   - Use template: `.gemini/templates/project-analysis-tmpl.yaml`
...
2. For Each Agent (one at a time):
   ...
   - Follow optimization task: `.gemini/tasks/optimize-agent.yaml`
...
3. Load Best Practices: Use `.gemini/data/optimization-best-practices.md`
...
1. For Each Task (one at a time):
   ...
   - Follow optimization task: `.gemini/tasks/optimize-task.yaml`
   - Create diff showing proposed changes
   - Present to user with rationale
   - Wait for approval
   - Apply if approved
...
1. For Each Checklist (one at a time):
   ...
   - Follow optimization task: `.gemini/tasks/optimize-checklist.yaml`
   - Create diff showing proposed changes
   - Present to user with rationale
   - Wait for approval
   - Apply if approved
...
Files to Optimize:
- Agents: `.gemini/agents/*.md` (with user approval)
- Tasks: `.gemini/tasks/*.yaml` (with user approval)
- Checklists: `.gemini/checklists/*.yaml` (with user approval)

Forbidden:
- Never write to `.gemini/` without explicit user approval via the [1] Apply option
...
- `.gemini/agents/prepper/SOUL.md` (identity: personality, tone, constraints)
- `.gemini/agents/prepper/SKILLS.md` (skills: capabilities, tasks, checklists)
- `.gemini/agents/prepper/SUBAGENTS.md` (subagents: delegation map, Task templates)
- `.gemini/agents/prepper/DUTIES.md` (duties: deliverables, SLAs, definition of done)
- `.gemini/agents/prepper/rules/` (rules: enforcement hard stops)
- `.gemini/agents/prepper/memory/` (memory: session state and handoff pointers)
You have access to these resources:
- Analyze project context task: `.gemini/tasks/analyze-project-context.yaml`
- Optimize agent task: `.gemini/tasks/optimize-agent.yaml`
- Optimize task workflow: `.gemini/tasks/optimize-task.yaml`
- Optimize checklist task: `.gemini/tasks/optimize-checklist.yaml`
- Project analysis template: `.gemini/templates/project-analysis-tmpl.yaml`
- Optimization best practices: `.gemini/data/optimization-best-practices.md`
