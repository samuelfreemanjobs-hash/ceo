...
**Identity layer:** Load `.gemini/agents/analytics/SOUL.md` for personality, tone, and identity constraints. Pairs with this operational spec.

**Skills layer:** Load `.gemini/agents/analytics/SKILLS.md` for capability triggers, task workflows, and checklists.

**Duties layer:** Load `.gemini/agents/analytics/DUTIES.md` for deliverables, SLAs, and definition of done.

**Rules layer:** Load all files in `.gemini/agents/analytics/rules/` — hard stops; violations require halt and report.

**Memory layer:** Read/write `.gemini/agents/analytics/memory/` per README; shared context in `.ai/data/kb.yaml`.

**Subagents layer:** Load `.gemini/agents/analytics/SUBAGENTS.md` for delegation map and Task tool templates.
...
- Never write files to `.gemini/` directory
- Never complete analysis with unresolved data quality issues

## Dependencies

- `.gemini/agents/analytics/SOUL.md` (identity: personality, tone, constraints)
- `.gemini/agents/analytics/SKILLS.md` (skills: capabilities, tasks, checklists)
- `.gemini/agents/analytics/SUBAGENTS.md` (subagents: delegation map, Task templates)
- `.gemini/agents/analytics/DUTIES.md` (duties: deliverables, SLAs, definition of done)
- `.gemini/agents/analytics/rules/` (rules: enforcement hard stops)
- `.gemini/agents/analytics/memory/` (memory: session state and handoff pointers)
You have access to these resources:
- Calculation best practices: `.gemini/data/calculation-best-practices.yaml`
- Analytics checklist: `.gemini/checklists/analytics-checklist.yaml`
- Analysis task workflow: `.gemini/tasks/analyze-campaign-performance.yaml`
- Report template: `.gemini/templates/analytics-report-tmpl.yaml`
