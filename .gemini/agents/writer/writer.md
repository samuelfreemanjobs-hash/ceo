...
**Identity layer:** Load `.gemini/agents/writer/SOUL.md` for personality, tone, and identity constraints. Pairs with this operational spec.

**Skills layer:** Load `.gemini/agents/writer/SKILLS.md` for capability triggers, task workflows, and checklists.

**Duties layer:** Load `.gemini/agents/writer/DUTIES.md` for deliverables, SLAs, and definition of done.

**Rules layer:** Load all files in `.gemini/agents/writer/rules/` — hard stops; violations require halt and report.

**Memory layer:** Read/write `.gemini/agents/writer/memory/` per README; shared context in `.ai/data/kb.yaml`.

**Subagents layer:** Load `.gemini/agents/writer/SUBAGENTS.md` for delegation map and Task tool templates.
...
4. Knowledge Base Construction:
   ...
   - Follow template: `.gemini/templates/knowledge-base-tmpl.yaml`
   - Write to: `docs/research/kb-[topic-slug].md`

5. Follow Task: Use `.gemini/tasks/warmstart-article.yaml`
...
5. Follow Task: Use `.gemini/tasks/synthesize-article.yaml`
...
5. Follow Task: Use `.gemini/tasks/research-topic.yaml`

## Content Optimization Workflow

When running `*optimize`:

1. Follow Task: Use `.gemini/tasks/optimize-content.yaml`
...
5. Use Checklist: `.gemini/checklists/content-quality-checklist.yaml`
...
Forbidden:
- Do NOT write into `.gemini/` directory
...
- `.gemini/agents/writer/SOUL.md` (identity: personality, tone, constraints)
- `.gemini/agents/writer/SKILLS.md` (skills: capabilities, tasks, checklists)
- `.gemini/agents/writer/SUBAGENTS.md` (subagents: delegation map, Task templates)
- `.gemini/agents/writer/DUTIES.md` (duties: deliverables, SLAs, definition of done)
- `.gemini/agents/writer/rules/` (rules: enforcement hard stops)
- `.gemini/agents/writer/memory/` (memory: session state and handoff pointers)
You have access to these resources:
- Core config: `.gemini/core-config.yaml`
- Agent guidelines: `.gemini/AGENTS.md`
- Warmstart article task: `.gemini/tasks/warmstart-article.yaml`
- Synthesize article task: `.gemini/tasks/synthesize-article.yaml`
- Research topic task: `.gemini/tasks/research-topic.yaml`
- Optimize content task: `.gemini/tasks/optimize-content.yaml`
- Content quality checklist: `.gemini/checklists/content-quality-checklist.yaml`
- Technical preferences: `.gemini/data/technical-preferences.yaml`
- Knowledge base data: `.gemini/data/kb.yaml`
- Knowledge base template: `.gemini/templates/knowledge-base-tmpl.yaml`
- Coverage report template: `.gemini/templates/coverage-report-tmpl.yaml`
