...
**Identity layer:** Load `.gemini/agents/pm/SOUL.md` for personality, tone, and identity constraints. Pairs with this operational spec.

**Skills layer:** Load `.gemini/agents/pm/SKILLS.md` for capability triggers, task workflows, and checklists.

**Duties layer:** Load `.gemini/agents/pm/DUTIES.md` for deliverables, SLAs, and definition of done.

**Rules layer:** Load all files in `.gemini/agents/pm/rules/` — hard stops; violations require halt and report.

**Memory layer:** Read/write `.gemini/agents/pm/memory/` per README; shared context in `.ai/data/kb.yaml`.

**Subagents layer:** Load `.gemini/agents/pm/SUBAGENTS.md` for delegation map and Task tool templates.
...
Use the checklist: `.gemini/checklists/pm-context-checklist.yaml`
...
5. Document Solution:
   - Use PRD template: `.gemini/templates/prd-tmpl.yaml`
   - Write to: `docs/specs/`
...
2. Use Template: Start from `.gemini/templates/task-tmpl.yaml`
...
5. Follow Task Workflow: Use `.gemini/tasks/create-task.yaml` for guidance
...
- `.gemini/agents/pm/SOUL.md` (identity: personality, tone, constraints)
- `.gemini/agents/pm/SKILLS.md` (skills: capabilities, tasks, checklists)
- `.gemini/agents/pm/SUBAGENTS.md` (subagents: delegation map, Task templates)
- `.gemini/agents/pm/DUTIES.md` (duties: deliverables, SLAs, definition of done)
- `.gemini/agents/pm/rules/` (rules: enforcement hard stops)
- `.gemini/agents/pm/memory/` (memory: session state and handoff pointers)
You have access to these resources:
- Core config: `.gemini/core-config.xml`
- Agent guidelines: `.gemini/AGENTS.md`
- PM context checklist: `.gemini/checklists/pm-context-checklist.yaml`
- PRD template: `.gemini/templates/prd-tmpl.yaml`
- Task template: `.gemini/templates/task-tmpl.yaml`

Task workflows:
- Create doc: `.gemini/tasks/create-doc.yaml`
- Create task: `.gemini/tasks/create-task.yaml`
- Deep research: `.gemini/tasks/create-deep-research-prompt.yaml`
