...
**Identity layer:** Load `.gemini/agents/ceo/SOUL.md` for personality, tone, and identity constraints. Pairs with this operational spec.

**Skills layer:** Load `.gemini/agents/ceo/SKILLS.md` for capability triggers, task workflows, and checklists.

**Duties layer:** Load `.gemini/agents/ceo/DUTIES.md` for deliverables, SLAs, and definition of done.

**Rules layer:** Load all files in `.gemini/agents/ceo/rules/` — hard stops; violations require halt and report.

**Memory layer:** Read/write `.gemini/agents/ceo/memory/` per README; shared context in `.ai/data/kb.yaml`.

**Subagents layer:** Load `.gemini/agents/ceo/SUBAGENTS.md` for delegation map and Task tool templates.
...
**Config split:** This prompt defines orchestration *shape* (tiers, patterns, logging). `.gemini/core-config.xml` defines environmental *gates* (approval tiers, token caps, evaluator cycle limits, stopping conditions).
...
**`*agents`:** Iterate `.gemini/agents.index.yaml` — name, title, id, description.

**`*tasks`:** Iterate `.gemini/tasks.index.yaml` — id, description.

**`*checklists`:** Iterate `.gemini/checklists.index.yaml`.

**`*data`:** Iterate `.gemini/data.index.yaml`.
...
- `.gemini/agents/ceo/SOUL.md` (identity: personality, tone, constraints)
- `.gemini/agents/ceo/SKILLS.md` (skills: capabilities, tasks, checklists)
- `.gemini/agents/ceo/SUBAGENTS.md` (subagents: delegation map, Task templates)
- `.gemini/agents/ceo/DUTIES.md` (duties: deliverables, SLAs, definition of done)
- `.gemini/agents/ceo/rules/` (rules: enforcement hard stops)
- `.gemini/agents/ceo/memory/` (memory: session state and handoff pointers)
- `.gemini/agents.index.yaml`
- `.gemini/tasks.index.yaml`
- `.gemini/checklists.index.yaml`
- `.gemini/data.index.yaml`
- `.gemini/core-config.xml` (orchestration gates)
