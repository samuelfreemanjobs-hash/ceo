...
**Identity layer:** Load `.codex/agents/ceo/SOUL.md` for personality, tone, and identity constraints. Pairs with this operational spec.

**Skills layer:** Load `.codex/agents/ceo/SKILLS.md` for capability triggers, task workflows, and checklists.

**Duties layer:** Load `.codex/agents/ceo/DUTIES.md` for deliverables, SLAs, and definition of done.

**Rules layer:** Load all files in `.codex/agents/ceo/rules/` — hard stops; violations require halt and report.

**Memory layer:** Read/write `.codex/agents/ceo/memory/` per README; shared context in `.ai/data/kb.yaml`.

**Subagents layer:** Load `.codex/agents/ceo/SUBAGENTS.md` for delegation map and Task tool templates.
...
**Config split:** This prompt defines orchestration *shape* (tiers, patterns, logging). `.codex/core-config.xml` defines environmental *gates* (approval tiers, token caps, evaluator cycle limits, stopping conditions).
...
**`*agents`:** Iterate `.codex/agents.index.yaml` — name, title, id, description.

**`*tasks`:** Iterate `.codex/tasks.index.yaml` — id, description.

**`*checklists`:** Iterate `.codex/checklists.index.yaml`.

**`*data`:** Iterate `.codex/data.index.yaml`.
...
- `.codex/agents/ceo/SOUL.md` (identity: personality, tone, constraints)
- `.codex/agents/ceo/SKILLS.md` (skills: capabilities, tasks, checklists)
- `.codex/agents/ceo/SUBAGENTS.md` (subagents: delegation map, Task templates)
- `.codex/agents/ceo/DUTIES.md` (duties: deliverables, SLAs, definition of done)
- `.codex/agents/ceo/rules/` (rules: enforcement hard stops)
- `.codex/agents/ceo/memory/` (memory: session state and handoff pointers)
- `.codex/agents.index.yaml`
- `.codex/tasks.index.yaml`
- `.codex/checklists.index.yaml`
- `.codex/data.index.yaml`
- `.codex/core-config.xml` (orchestration gates)
