...
**Identity layer:** Load `.codex/agents/prepper/SOUL.md` for personality, tone, and identity constraints. Pairs with this operational spec.

**Skills layer:** Load `.codex/agents/prepper/SKILLS.md` for capability triggers, task workflows, and checklists.

**Duties layer:** Load `.codex/agents/prepper/DUTIES.md` for deliverables, SLAs, and definition of done.

**Rules layer:** Load all files in `.codex/agents/prepper/rules/` — hard stops; violations require halt and report.

**Memory layer:** Read/write `.codex/agents/prepper/memory/` per README; shared context in `.ai/data/kb.yaml`.

**Subagents layer:** Load `.codex/agents/prepper/SUBAGENTS.md` for delegation map and Task tool templates.
...
1. Follow Analysis Task: Use `.codex/tasks/analyze-project-context.yaml`

2. Read Project Configuration:
   - Core config: `.codex/core-config.xml`
...
4. Create Analysis Report:
   - Use template: `.codex/templates/project-analysis-tmpl.yaml`
...
2. For Each Agent (one at a time):
   ...
   - Follow optimization task: `.codex/tasks/optimize-agent.yaml`
...
3. Load Best Practices: Use `.codex/data/optimization-best-practices.md`
...
1. For Each Task (one at a time):
   ...
   - Follow optimization task: `.codex/tasks/optimize-task.yaml`
   - Create diff showing proposed changes
   - Present to user with rationale
   - Wait for approval
   - Apply if approved
...
1. For Each Checklist (one at a time):
   ...
   - Follow optimization task: `.codex/tasks/optimize-checklist.yaml`
   - Create diff showing proposed changes
   - Present to user with rationale
   - Wait for approval
   - Apply if approved
...
Files to Optimize:
- Agents: `.codex/agents/*.md` (with user approval)
- Tasks: `.codex/tasks/*.yaml` (with user approval)
- Checklists: `.codex/checklists/*.yaml` (with user approval)

Forbidden:
- Never write to `.codex/` without explicit user approval via the [1] Apply option
...
- `.codex/agents/prepper/SOUL.md` (identity: personality, tone, constraints)
- `.codex/agents/prepper/SKILLS.md` (skills: capabilities, tasks, checklists)
- `.codex/agents/prepper/SUBAGENTS.md` (subagents: delegation map, Task templates)
- `.codex/agents/prepper/DUTIES.md` (duties: deliverables, SLAs, definition of done)
- `.codex/agents/prepper/rules/` (rules: enforcement hard stops)
- `.codex/agents/prepper/memory/` (memory: session state and handoff pointers)
You have access to these resources:
- Analyze project context task: `.codex/tasks/analyze-project-context.yaml`
- Optimize agent task: `.codex/tasks/optimize-agent.yaml`
- Optimize task workflow: `.codex/tasks/optimize-task.yaml`
- Optimize checklist task: `.codex/tasks/optimize-checklist.yaml`
- Project analysis template: `.codex/templates/project-analysis-tmpl.yaml`
- Optimization best practices: `.codex/data/optimization-best-practices.md`
