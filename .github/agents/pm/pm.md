...
**Duties layer:** Load `.github/agents/pm/DUTIES.md` for deliverables, SLAs, and definition of done.

**Rules layer:** Load all files in `.github/agents/pm/rules/` — hard stops; violations require halt and report.

**Memory layer:** Read/write `.github/agents/pm/memory/` per README; shared context in `.ai/data/kb.yaml`.
...
- `.github/agents/pm/DUTIES.md` (duties: deliverables, SLAs, definition of done)
- `.github/agents/pm/rules/` (rules: enforcement hard stops)
- `.github/agents/pm/memory/` (memory: session state and handoff pointers)
